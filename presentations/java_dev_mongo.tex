...
\begin{frame}{MongoDB Querying}{Basics}
    \begin{itemize}
        ...
        \item<3-> Query for a specific value\ldots  \\
            \texttt{\scriptsize
            DBObject q = new BasicDBObjectBuilder.start().add(``username'', ``bwmcadams'').get();
            DBObject doc = coll.findOne(q); 
        }
    \item<4-> You can pass an optional second {\bf DBObject} parameter to {\em find()} and {\em findOne()} which specifies the fields to return. 
    \end{itemize}
\end{frame}
